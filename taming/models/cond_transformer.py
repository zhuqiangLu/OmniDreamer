...
class Net2NetTransformer(pl.LightningModule):
    def __init__(self, transformer_config, first_stage_config,
        ...
        if False:
            # Set True, if you wanna overwrite with a new VQGAN codebook upon a codebook including in Trans's ckpt_path
            # For example, fine tuning with Laval indoor dataset on the model trained with SUN360.
            self.init_first_stage_from_ckpt(first_stage_config)
            self.init_cond_stage_from_ckpt(cond_stage_config)
    ...
    def log_images(self, batch, temperature=None, top_k=None, callback=None, lr_interface=False, **kwargs):
        ...
        if self.cond_stage_key != "image":
            ...
            if self.cond_stage_key == "concat_input":
                log["conditioning_rec"] = cond_rec[:,:3,:,:]
                log["conditioning"] = c[:,:3,:,:]
            else:
